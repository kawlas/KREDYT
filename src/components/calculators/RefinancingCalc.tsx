    ...
    >
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-start">
        ...
        <div className="space-y-6">
          <Card>
            <h2 className="text-xl font-semibold text-foreground mb-6 border-b pb-4">Obecny kredyt</h2>
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-foreground mb-1">Pierwotna kwota (PLN)</label>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-foreground mb-1">Miesięcy spłaconych</label>
          ...
          <Card>
            <h2 className="text-xl font-semibold text-foreground mb-6 border-b pb-4">Nowa oferta</h2>
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-foreground mb-1">Wybierz bank (lub wpisz ręcznie)</label>
              ...
              <div>
                <label className="block text-sm font-medium text-foreground mb-1">Nowe oprocentowanie (%)</label>
                <input type="number" step="0.01" value={newRate} onChange={e => setNewRate(Number(e.target.value))} min={0}
                  className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-ring outline-none" />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-foreground mb-1">Koszty dodatkowe (wpisane ręcznie)</label>
                  <input type="number" value={transferFees} onChange={e => setTransferFees(Number(e.target.value))} min={0}
                    className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-ring outline-none" />
                  <p className="text-xs text-muted-foreground mt-1">Suma: wycena (~500 zł) + notariusz (~1000-3000 zł) + wpis do KW (200 zł) + wykreślenie hipoteki (100 zł)</p>
                </div>
                <div>
                  <label className="block text-sm font-medium text-foreground mb-1">Prowizja nowego banku (%)</label>
                  <input type="number" step="0.1" value={newProvision * 100} onChange={e => setNewProvision(Number(e.target.value) / 100)} min={0}
                    className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-ring outline-none" />
                </div>
              </div>
              <div>
                <label className="block text-sm font-medium text-foreground mb-1">Opłata za wcześniejszą spłatę (%)</label>
                <input type="number" step="0.1" value={earlyFee} onChange={e => setEarlyFee(Number(e.target.value))} min={0} max={5}
                  className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-ring outline-none" />
                <p className="text-xs text-muted-foreground mt-1">Przy zmiennym oprocentowaniu: 0% po 1. roku</p>
              </div>

              <details className="border-t pt-4">
                <summary className="text-sm font-medium text-muted-foreground cursor-pointer hover:text-foreground">Opcje zaawansowane</summary>
                <div className="mt-4 space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="block text-sm font-medium text-foreground mb-1">Dzień rozliczenia</label>
                    ...
                    <div>
                      <label className="block text-sm font-medium text-foreground mb-1">Dzień ostatniej raty</label>
                  ...
                  <div>
                    <label className="block text-sm font-medium text-foreground mb-1">Nowy okres (lat) — opcjonalnie</label>
                    <input type="number" value={newTermYears} onChange={e => setNewTermYears(Math.max(0, Number(e.target.value)))} min={0}
                      className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-ring outline-none" />
                    <p className="text-xs text-muted-foreground mt-1">Pozostaw 0, aby zachować pozostały okres kredytowania</p>
                  </div>
                  <label className="flex items-center gap-2 text-sm cursor-pointer">
                    <input type="checkbox" checked={capitalizeCosts} onChange={e => setCapitalizeCosts(e.target.checked)}
                      className="w-4 h-4 rounded border-border text-primary focus:ring-ring" />
                    <span className="font-medium text-foreground">Dodaj koszty do kapitału nowego kredytu</span>
                  </label>
                  <p className="text-xs text-muted-foreground">Koszty refinansowania doliczane do salda — niższy wydatek z góry, ale wyższa rata.</p>
                </div>
              </details>
            </div>
          </Card>
              ...
              <Card title="Wyniki refinansowania">
                <div className="space-y-4">
                  <div className="bg-primary/10 p-4 rounded-lg">
                    <div className="text-sm text-muted-foreground mb-1">Saldo do spłaty</div>
                    <div className="text-2xl font-bold text-foreground">{formatCurrencyShort(result.remainingBalance)}</div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="bg-green-50 p-4 rounded-lg">
                      <div className="text-sm text-muted-foreground mb-1">Miesięczna oszczędność</div>
                    ...
                    <div className="bg-yellow-50 p-4 rounded-lg">
                      <div className="text-sm text-muted-foreground mb-1">{capitalizeCosts ? 'Dodane do kapitału' : 'Koszty z góry'}</div>
                  ...
                  <div className="border-t pt-4 space-y-1">
                    <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide mt-2">Wpisane ręcznie</p>
                    ...
                    <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide mt-3">Wyliczone automatycznie</p>
                    ...
                    <div className="flex justify-between text-sm border-t pt-2 mt-3">
                      <span className="text-foreground font-semibold">Całkowite koszty refinansowania:</span>
                      <span className="font-bold">{formatCurrencyShort(result.totalCosts)}</span>
                    </div>
                    <div className="flex justify-between text-sm">
                      <span className="text-muted-foreground">Oszczędność na odsetkach:</span>
                      <span className="font-bold text-green-600">{formatCurrencyShort(result.interestSaved)}</span>
                    </div>
                    <div className="flex justify-between text-sm border-t pt-2">
                      <span className="text-foreground font-semibold">Korzyść netto:</span>
