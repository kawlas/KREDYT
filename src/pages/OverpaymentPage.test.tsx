...
import OverpaymentPage from './OverpaymentPage'

describe('OverpaymentPage - H1', () => {
  const renderPage = () => {
    render(
      <HelmetProvider>
        <MemoryRouter>
  ...
  it('h1 zawiera "nadpłat" lub "overpayment"', () => {
    renderPage()
    const h1 = screen.getByRole('heading', { level: 1 })
    const text = h1.textContent!.toLowerCase()
    const hasExpected = text.includes('nadpłat') || text.includes('overpayment')
